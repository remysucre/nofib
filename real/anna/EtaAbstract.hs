...
eaEtaAbstract :: AnnExpr Naam TExpr -> AnnExpr Naam TExpr
...
eaEtaAbstract (ae@(!(tau, ALet rf (defs) body)))
  = let typeInfo = [eaUncurry ty | (n, (!((!(ty)), rhs))) <- defs]
        ...
        fixOne sc@(((!(n)), (!(tau, non_lam_b)))) ((!(!argTs)), resT)
          | null argTs = sc
          | otherwise =
            eaMain (n, (tau, ALam [] (tau, non_lam_b))) argTs resT
...
eaMain ::
       (Naam, AnnExpr Naam TExpr) ->
         [TExpr] -> TExpr -> (Naam, AnnExpr Naam TExpr)
...
eaMakeApChain ::
              [((Naam, TExpr), TExpr)] ->
                AnnExpr Naam TExpr -> AnnExpr Naam TExpr
...
eaCurry resT ([]) = resT
eaCurry resT ((((!(argT : argTs)))))
  = TArr argT (eaCurry resT argTs)
 
eaUncurry :: TExpr -> ([TExpr], TExpr)
eaUncurry (!(TVar tv)) = ([], TVar tv)
eaUncurry (TArr t1 t2)
  = let (rest, final) = eaUncurry t2 in (t1 : rest, final)
eaUncurry ((!(TCons tcon ((!targs))))) = ([], TCons tcon targs)