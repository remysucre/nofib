...
ppPrintCExprMain (EAp e1 e2)
  = (ppPrintLAp e1) `utiAppend`
      ((utiStr " ") `utiAppend` (ppPrintRAp e2))
      ...
      (rec `utiAppend`
         ((utiIndent
             (utiInterleave (utiStr ";\n")
                [(utiStr n) `utiAppend`
                   ((utiStr " = ") `utiAppend` (ppPrintCExprMain e))
                 | (n, e) <- ds]))
...
ppPrintCExprMain (ECase (!(sw)) al)
  = (utiStr "case ") `utiAppend`
      ((ppPrintCExprMain sw) `utiAppend`
         ((utiStr " of\n") `utiAppend`
            ((utiIndent (utiInterleave (utiStr ";\n") (map ppPrintAlter al)))
               `utiAppend` (utiStr "\nend"))))
ppPrintAlter (cn, (cal, cexp))
  = (utiStr "  ") `utiAppend`
      ((utiStr cn) `utiAppend`
         ((utiStr " ") `utiAppend`
            ((utiInterleave (utiStr " ") [(utiStr ca) | ca <- cal]) `utiAppend`
               ((utiStr " -> ") `utiAppend`
                  ((utiIndent (ppPrintCExprMain cexp)))))))
ppPrintRAp ((!(EVar v))) = utiStr v
ppPrintRAp ((!(ENum (!n)))) = utiNum n
ppPrintRAp ((!(EConstr c))) = utiStr c
ppPrintRAp e
  = (utiStr "(") `utiAppend`
      ((ppPrintCExprMain e) `utiAppend` (utiStr ")"))
ppPrintLAp (!(EVar (!v))) = utiStr v
ppPrintLAp (ENum n) = utiNum n
ppPrintLAp (((!(EConstr c)))) = utiStr c
ppPrintLAp (EAp e1 e2)
  = (ppPrintLAp e1) `utiAppend`
      ((utiStr " ") `utiAppend` (ppPrintRAp e2))
ppPrintLAp e
  = (utiStr "(") `utiAppend`
