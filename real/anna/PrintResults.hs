...
prAllPoints d = "{" ++ interleave " " ((h . g . f) d) ++ "}"
  where f Two = [[(-1) :: Int], [0 :: Int]]
...
prRoute (!d) r
  = let  
        k :: Int -> Char
        k n = toEnum (n + 48)
      in map k (prRouteMain d r)
 
prRouteMain :: Domain -> Route -> [Int]
prRouteMain (!Two) Zero = [0 :: Int]
prRouteMain (!Two) One = [1 :: Int]
prRouteMain (d@((!(Lift1 ds)))) Stop1 = copy (prWidth d) 0
prRouteMain d@(Lift1 ds) (Up1 rs)
  = map (prSucc 1) (prRouteMain_cross ds rs)
prRouteMain d@((!(Lift2 ds))) Stop2 = copy (prWidth d) 0
prRouteMain d@((Lift2 (ds))) Up2 = copy (prWidth d) 1
prRouteMain d@((!(Lift2 ds))) (((!(UpUp2 rs))))
  = map (prSucc 2) (prRouteMain_cross ds rs)
prRouteMain_cross ds rs = concat fixedRoutes
  where unFixedRoutes = myZipWith2 prRouteMain ds rs
        compFactors = map prLiftsIn ds
        compFactMax = maximum compFactors
        (!compFactNorm) = map subCompFactMax compFactors
        fixedRoutes
          = map applyCompensationFactor (myZip2 compFactNorm unFixedRoutes)
        applyCompensationFactor (n, roote) = map (prSucc n) roote
         
        subCompFactMax :: Int -> Int
        subCompFactMax nn = compFactMax - nn
 
prPrintFunction ::
                Bool -> StaticComponent -> Naam -> Point -> [Char]
prPrintFunction mi statics fName
  (fDomain@((Func dss dt)), (!(Rep rep)))
  ...
  where pseudoParams
          = utSureLookup (utSCfreevars statics) "prPrintFunction" fName ++
              forever ""
        ...
        f op
          = let ipl = inMinInverse mi fDomain (Rep rep) op in
              (copy (8 - length outColText) ' ') ++
                outColText ++ "   |  " ++ (interleave " and " (map g ipl)) ++ "\n"
          where outColText = prRoute dt op
