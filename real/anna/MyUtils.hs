...
myZipWith3 f (![]) [] [] = []
myZipWith3 f ((!(!a)) : (!as)) (b : bs) (c : (!cs))
  = f a b c : myZipWith3 f as bs cs
...
myZipWith4 ::
           (a -> b -> c -> d -> e) -> [a] -> [b] -> [c] -> [d] -> [e]
myZipWith4 (!f) [] (![]) [] [] = []
myZipWith4 f ((!(a : (as)))) (b : bs) ((c : (!cs))) ((d : (!ds)))
  = f a b c d : myZipWith4 f as bs cs ds
...
myZipWith5 ::
           (a -> b -> c -> d -> e -> f) ->
             [a] -> [b] -> [c] -> [d] -> [e] -> [f]
myZipWith5 f [] [] [] [] [] = []
myZipWith5 (!f) (a : as) (b : bs) ((c : (!cs))) (d : (!ds))
  (((!(e : (!es))))) = f a b c d e : myZipWith5 f as bs cs ds es
myZipWith5 _ _ _ (!_) _ _ = panic "myZipWith5: unequal lists"
myZip5 = myZipWith5 (\ a b (!c) d e -> (a, b, c, d, e))
 
myAndWith2 :: (a -> b -> Bool) -> [a] -> [b] -> Bool
myAndWith2 f [] [] = True
myAndWith2 (!(!f)) ((a : (!as))) (b : bs)
  = if f a b then myAndWith2 f as bs else False
myAndWith2 _ (!_) _ = panic "myAndWith2: unequal lists"
...
myIntsFromTo n m
  = if n > m then [] else n : myIntsFromTo (n + (1 :: Int)) m
