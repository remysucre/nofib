...
txGetInstantiations :: DExpr -> DExpr -> AList Naam Domain
txGetInstantiations (!simplest) usage
  = consistent [] (gi simplest usage)
  where gi (DXVar v) dexpr = [(v, dxApplyDSubst_2 dexpr)]
        gi DXTwo DXTwo = []
        ...
        gi (!(DXFunc dxss1 dxt1)) (!(!(DXFunc dxss2 (!dxt2))))
          = let basis_arity = length dxss1
                ...
                (new_dxss2, new_dxt2)
                  = if usage_arity > basis_arity then
                      (take basis_arity dxss2, DXFunc (drop basis_arity dxss2) dxt2) else
                      (dxss2, dxt2)
        ...
        consistent acc ((((!((!v), dx)) : (!rest))))
          = case utLookup acc v of
                ...
                (((!(Just (!dy))))) -> if dx == dy then consistent acc rest else
                                         panic "txGetInstantiations"
 
tx2dxAnnTree ::
             TypeDependancy -> AnnExpr Naam TExpr -> AnnExpr Naam DExpr
...
tx2dx (!td) texpr
  = let (!typeVars) = sort (nub (tcTvars_in texpr))
      ...
      in
      if length typeVars > 26 then panic "tx2dx" else
        dxNormaliseDExpr (tx2dx_aux td dVarEnv texpr)
tx2dx_aux td env ((!(TVar (v))))
  = DXVar (utSureLookup env "tx2dx_aux(1)" v)
tx2dx_aux td env (TCons "int" []) = DXTwo
tx2dx_aux td (!env) ((TCons "char" (![]))) = DXTwo
tx2dx_aux (!td) (!env) (TArr (!t1) t2)
  = DXFunc [tx2dx_aux td env t1] (tx2dx_aux td env t2)
tx2dx_aux (!td) env ((!(TCons tname (targs))))
  = if mdIsRecursiveType td tname then
      DXLift2 (map (tx2dx_aux td env) targs) else
      DXLift1 (map (tx2dx_aux td env) targs)