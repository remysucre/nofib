...
apPap_2 argCount (Min1Max0 ar f1 f0) args
  = let newf1
          = sort
              (avMinfrel
                 [MkFrel (drop argCount fel) | MkFrel fel <- f1,
                  myAndWith2 (<<) (take argCount fel) args])
        newf0
          = sort
              (avMaxfrel
                 [MkFrel (drop argCount fel) | MkFrel fel <- f0,
                  myAndWith2 (<<) args (take argCount fel)])
        result = Min1Max0 (ar - argCount) newf1 newf0
      in if argCount <= ar then result else panic "apPap_2"
 
apPap :: Rep -> [Route] -> Rep
apPap (RepTwo fr) args
  = let argCount = length args in RepTwo (apPap_2 argCount fr args)
...
apPapConst
  rep@(!((!(Rep2 (Min1Max0 lf_ar lf_f1 (!lf_f0))
              (Min1Max0 mf_ar (mf_f1) mf_f0) (!(hfs))))))
  | lf_ar > 0 = Rep rep
