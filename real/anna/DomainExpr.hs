...
dxApplyDSubst_2 (DXFunc dxs dxt)
  = Func (map dxApplyDSubst_2 dxs) (dxApplyDSubst_2 dxt)
 
dxApplyDSubst :: DSubst -> DExpr -> Domain
dxApplyDSubst rho DXTwo = Two
dxApplyDSubst rho (DXVar (!alpha))
  = utSureLookup rho "dxApplySubst" alpha
dxApplyDSubst rho ((!(DXLift1 (!(![]))))) = Two
dxApplyDSubst rho (DXLift1 dxs)
  = Lift1 (map (dxApplyDSubst rho) dxs)
dxApplyDSubst rho ((!(DXLift2 []))) = Lift1 [Two]
dxApplyDSubst rho (((!(DXLift2 (!dxs)))))
  = Lift2 (map (dxApplyDSubst rho) dxs)
dxApplyDSubst (!rho) ((!(DXFunc (!dxs) (dxt))))
  = Func (map (dxApplyDSubst rho) dxs) (dxApplyDSubst rho dxt)
...
dxNormaliseDExpr (((!(DXLift2 dxs))))
  = DXLift2 (map dxNormaliseDExpr dxs)
dxNormaliseDExpr (DXVar v) = DXVar v
 
dxContainsFnSpace :: DExpr -> Bool
dxContainsFnSpace (!DXTwo) = False
dxContainsFnSpace (!(((!((!(DXLift1 dxs)))))))
...
dxContainsSubsidiaryFnSpace DXTwo = False
...
dxContainsSubsidiaryFnSpace (DXVar _) = False
 
dxDiff :: Domain -> Domain -> (DExpr, DSubst)
dxDiff db (!(!ds))
  = case doStatefulOp2 dxDiff_aux (fromEnum 'a', []) ds db of
        (!(dexpr, (num, dsubst))) -> (dexpr, dsubst)
dxDiff_aux Two Two = returnS DXTwo
...
dxDiff_aux (!(Lift1 ds1)) (Lift1 ds2)
  = dxDiff_list ds1 ds2 `thenS`
      (\ new_ds1_ds2 -> returnS (DXLift1 new_ds1_ds2))
dxDiff_aux (Lift2 ds1) ((!(Lift2 ((!ds2)))))
  = dxDiff_list ds1 ds2 `thenS`
      (\ new_ds1_ds2 -> returnS (DXLift2 new_ds1_ds2))
dxDiff_aux (Func (dss1) (!dt1)) (Func (!dss2) dt2)
  = dxDiff_list dss1 dss2 `thenS`
      (\ new_dss1_dss2 ->
         dxDiff_aux dt1 dt2 `thenS`
           (\ new_dt1_dt2 -> returnS (DXFunc new_dss1_dss2 new_dt1_dt2)))
dxDiff_aux other1 other2 = panic "dxDiff_aux"
dxDiff_list [] (!(![])) = returnS []
dxDiff_list (a : as) (b : (!bs))
  = dxDiff_aux a b `thenS`
      (\ new_a_b ->
         dxDiff_list as bs `thenS`
           (\ new_as_bs -> returnS (new_a_b : new_as_bs)))
dxDiff_list other1 (!other2) = panic "dxDiff_list: unequal lists"