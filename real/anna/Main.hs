...
maBaseAnns
  = [("_not", strictUnaryFunc), ("_+", strictBinaryFunc),
     ("_-", strictBinaryFunc), ("_*", strictBinaryFunc),
     ("_/", strictBinaryFunc), ("_%", strictBinaryFunc),
     ("_<", strictBinaryFunc), ("_<=", strictBinaryFunc),
     ("_==", strictBinaryFunc), ("_~=", strictBinaryFunc),
     ("_>=", strictBinaryFunc), ("_>", strictBinaryFunc),
     ("_|", strictBinaryFunc), ("_&", strictBinaryFunc),
     ("_#", nonLambdaDefinableFunc), ("False", HPoint One),
     ("True", HPoint One)]
  where strictUnaryFunc
          = HPoint (Rep (RepTwo (Min1Max0 1 [MkFrel [One]] [MkFrel [Zero]])))
        strictBinaryFunc
          = HPoint
              (Rep
                 (RepTwo
...
maKludgeFlags flags
  = if DryRun `elem` flags then
      bdDryRunSettings ++ flags ++ bdDefaultSettings else
      flags ++ bdDefaultSettings
                ...
                (ppPrintParsed prog) ++
                  "\n\n\n=============" ++
                    "\n=== Types ===" ++
                      "\n=============\n" ++
                        prettyTypes ++ "\n\n" ++ strictAnResults ++ "\n"
  where flags = maKludgeFlags flagsInit
        strictAnResults
          = if Typecheck `notElem` flags then
              saMain (eaEtaAbstract typedTree) darAug fullEnvAug pseudoParams
                maBaseAnns
                tdsAug
                flags
                table
        ...
        (progAfterLL, pseudoParams) = llMain builtInNames expr doPretty
        ...
        f (words, Ok (rootTree, fullEnv)) = (words, rootTree, fullEnv)
...
maGetFlags [] = []
maGetFlags ((((!("-fTypecheck" : fs)))))
  = Typecheck : maGetFlags fs
...
maGetFlags ((((!("-fNoBaraki" : (!fs))))))
  = NoBaraki : maGetFlags fs
...
maGetFlags ((!((!other) : (!_))))
  = myFail ("Unknown flag: " ++ other ++ maUsage)
