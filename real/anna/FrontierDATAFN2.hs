...
fdImprove (!coerce) memo dss naive max0_super min1_super
  = let (zero_result_pairs, one_result_pairs)
          = splitList (fdIsZero . coerce . second) memo
        zero_nodes_max = avMaxfrel (map (MkFrel . first) zero_result_pairs)
        one_nodes_min = avMinfrel (map (MkFrel . first) one_result_pairs)
        new_max0
          = (if naive then [MkFrel (map avTopR dss)] else max0_super)
              `avLUBmax0frontier` (spMax0FromMin1 dss one_nodes_min)
        new_min1
          = (if naive then [MkFrel (map avBottomR dss)] else min1_super)
              `avGLBmin1frontier` (spMin1FromMax0 dss zero_nodes_max)
      in (new_max0, new_min1)
           ...
           Domain ->
             [Domain] ->
               [Domain] ->
                 Rep ->
                   (Route -> Route) ->
                     Bool -> [FrontierElem] -> MemoList -> (Rep, MemoList)
fdFind s_or_l hexpr (Func dss Two) small_argds big_argds
  (!(!(RepTwo (Min1Max0 ar_prev (!min1_prev) (!max0_prev))))) coerce
  naive min1_ilf memo
  = let (better_max0, better_min1)
          = fdImprove coerce memo small_argds naive max0_prev min1_prev
        (fr, (!new_memo_additions))
          = fdFs2 s_or_l hexpr small_argds big_argds (better_min1)
              better_max0
              coerce
      in (RepTwo fr, new_memo_additions)
fdFind s_or_l (!hexpr) (((Func (!(dss)) (!(Lift1 dts)))))
  small_argds (!big_argds)
  (((!(Rep1 (Min1Max0 ar_prev (!min1_prev_lf) max0_prev_lf)
         prev_hfs))))
  coerce naive min1_ilf memo
  = let (better_lf_max0, better_lf_min1)
          = fdImprove (fdLo1 . coerce) memo small_argds naive max0_prev_lf
              min1_prev_lf
        (lofact, (!(lofact_memo_additions)))
          = fdFs2 s_or_l hexpr small_argds big_argds (better_lf_min1)
              better_lf_max0
              (fdLo1 . coerce)
        useful_lofact_memo
        ...
        min1_lofact
          = case lofact of
                (!(Min1Max0 (!(lf_ar)) (!lf_f1) (!(lf_f0)))) -> lf_f1
...
fdFind (!s_or_l) hexpr ((!(Func dss (Lift2 (dts))))) small_argds
  big_argds
  (!((Rep2 (Min1Max0 (!ar_prev_lf) min1_prev_lf max0_prev_lf)
        (Min1Max0 (!ar_prev_mf) min1_prev_mf max0_prev_mf) prev_hfs)))
  coerce naive min1_ilf memo
  = let ((!better_lf_max0), better_lf_min1)
          = fdImprove (fdLo2 . coerce) memo small_argds naive max0_prev_lf
              min1_prev_lf
        ((!(lofact)), lofact_memo_additions)
          = fdFs2 s_or_l hexpr small_argds big_argds (better_lf_min1)
              better_lf_max0
        ...
        (midfact, midfact_memo_additions)
          = fdFs2 s_or_l hexpr small_argds big_argds (better_mf_min1)
              better_mf_max0
              (fdMid2 . coerce)
        useful_midfact_memo
        ...
        (!(!min1_midfact))
          = case midfact of
                ((Min1Max0 mf_ar (!mf_f1) mf_f0)) -> mf_f1
        (!(hifacts, hifact_memo_additions))
          = fdFind_aux s_or_l small_argds big_argds dts hexpr prev_hfs coerce
              useful_midfact_memo
              True
               ...
               [Domain] ->
                 [Domain] ->
                   HExpr Naam ->
                     [Rep] ->
                       (Route -> Route) ->
                         MemoList -> Bool -> [FrontierElem] -> Bool -> ([Rep], MemoList)
fdFind_aux s_or_l small_argds big_argds dts hexpr prev_hfs coerce
  initial_hf_memo (!double_lift) (!min1_ilf) naive
  = let high_coerce = if double_lift then fdHi2 else fdHi1
        small_hifact_domains = map (avUncurry small_argds) dts
        hifact_info_tuples
          = myZipWith4 mkTuple (0 `myIntsFromTo` (length dts - 1)) prev_hfs
              (map avBottomR dts)
        ...
        doOne memo_adds_so_far (hf_dom, (!(hf_coerce)), (!hf_preev))
          = let (rep, more_memo_additions)
                  = fdFind s_or_l hexpr hf_dom small_argds big_argds hf_preev
                      hf_coerce
                      naive
                      min1_ilf
                      (memo_adds_so_far ++ initial_hf_memo)
              in (more_memo_additions ++ memo_adds_so_far, rep)
      in (hifacts, hf_memo_additions)
      ...
      ACMode ->
        HExpr Naam ->
          [Domain] ->
            [Domain] ->
              [FrontierElem] ->
                [FrontierElem] -> (Route -> Route) -> (Frontier, MemoList)
fdFs2 s_or_l hexpr (!small_argds) big_argds (!min1_prev) max0_prev
  coerce
  = let initial_yy = max0_prev
        initial_xx = min1_prev
        (final_yy, (!((!final_xx))), finalMemo)
          = fdFs_aux s_or_l hexpr small_argds big_argds coerce initial_yy
              initial_xx
              True
              []
              (utRandomInts 1 2)
        result = Min1Max0 (length small_argds) final_xx final_yy
      in (result, finalMemo)
               ...
               [Domain] ->
                 (Route -> Route) ->
                   [FrontierElem] ->
                     [FrontierElem] ->
                       Bool ->
                         MemoList -> [Int] -> ([FrontierElem], [FrontierElem], MemoList)
fdFs_aux s_or_l hexpr (!small_argds) big_argds (coerce)
  trial_max_yy trial_min_xx fromTop memo rands
  = let edgez
          = fmSelect (head rands) trial_min_xx trial_max_yy fromTop
        (!(Just (MkFrel args))) = edgez
        args_at_proper_sizes
          = myZipWith3 (acConc s_or_l) big_argds small_argds args
        evald_app = aeEvalExact hexpr (map HPoint args_at_proper_sizes)
        coerced_evald_app = coerce evald_app
        revised_max_yy
          = fmReviseMaxYY small_argds trial_max_yy (MkFrel args)
        revised_min_xx
          = fmReviseMinXX small_argds trial_min_xx (MkFrel args)
        new_memo = (args, evald_app) : memo
      in
      if fmIsNothing edgez then
        (sort trial_max_yy, sort trial_min_xx, memo) else
        if coerced_evald_app == One then
          fdFs_aux s_or_l hexpr small_argds big_argds coerce revised_max_yy
            trial_min_xx
            False
            new_memo
            (tail rands)
          else
          if coerced_evald_app == Zero then
            fdFs_aux s_or_l hexpr small_argds big_argds coerce trial_max_yy
              revised_min_xx
              True
              new_memo
              (tail rands)
