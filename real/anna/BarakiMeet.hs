...
bmNorm (Lift2 (!ds)) ((((!(UpUp2 rs)))))
  = UpUp2 (myZipWith2 bmNorm ds rs)
bmNorm d (Rep rep) = Rep (bmNorm_rep d rep)
bmNorm_rep ((!(Func dss Two))) ((!(RepTwo fr)))
  = RepTwo (bmNorm_2 dss fr)
bmNorm_rep ((((!(Func (!(dss)) (!((!(Lift1 dts)))))))))
  (!(Rep1 lf ((!hfs))))
  = let hf_domains = map (avUncurry dss) dts in
      Rep1 (bmNorm_2 dss lf) (myZipWith2 bmNorm_rep hf_domains hfs)
...
bmNorm_2 dss (((!(Min1Max0 (!ar) f1 f0))))
  = let (!norm_f0) = sort (map (bmNorm_frel dss) f0)
      ...
      in Min1Max0 ar norm_f1 norm_f0
...
bmBelowEQrep ((!(Rep1 lf1 (!hfs1)))) (Rep1 lf2 hfs2)
  = bmBelowEQfrontier lf1 lf2 && myAndWith2 bmBelowEQrep hfs1 hfs2
bmBelowEQrep (Rep2 lf1 mf1 hfs1) (((Rep2 lf2 mf2 (!hfs2))))
  = bmBelowEQfrontier lf1 lf2 &&
      bmBelowEQfrontier mf1 mf2 && myAndWith2 bmBelowEQrep hfs1 hfs2
 
bmBelowEQfrontier :: Frontier -> Frontier -> Bool
bmBelowEQfrontier (!(!(Min1Max0 (!ar1) (!_) (!f0a))))
  (((Min1Max0 ar2 _ (!f0b))))
  = let outer [] = True
        outer ((((!(x : xs))))) = if inner x f0a then outer xs else False
        inner (!y) [] = False
        inner y (((!z) : (!zs)))
          = if y `bmBelowEQfrel` z then True else inner y zs
      in outer f0b