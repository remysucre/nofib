...
spSuccsR Two One = []
spSuccsR (Lift1 ds) Stop1 = [Up1 (map avBottomR ds)]
spSuccsR (Lift1 [d]) (Up1 [r])
  = map (\ rs -> Up1 [rs]) (spSuccsR d r)
spSuccsR (!(Lift1 ds)) (Up1 rs)
  = map Up1
      (myListVariants (map avBottomR ds) (myZipWith2 spSuccsR ds rs))
spSuccsR ((!(Lift2 (!(!ds))))) Stop2 = [Up2]
spSuccsR (Lift2 ds) Up2 = [UpUp2 (map avBottomR ds)]
spSuccsR (Lift2 (![d])) (UpUp2 (!([(!r)])))
  = map (\ (!rs) -> UpUp2 [rs]) (spSuccsR d r)
spSuccsR ((!(Lift2 ((!ds))))) (!(UpUp2 (!rs)))
  = map UpUp2
      (myListVariants (map avBottomR ds) (myZipWith2 spSuccsR ds rs))
spSuccsR (d@(Func _ (!_))) (Rep r) = map Rep (spSuccsRep d r)
...
spSuccsRep (Func (!dss) (!(Lift1 (!dts))))
  ...
  = let hfDomains = map (avUncurry dss) dts
        ...
        s2
          = [spLLift initTops dss hfDomains hfs2 |
...
spSuccsRep (Func dss (Lift2 dts)) ((!(Rep2 lf mf (!hfs))))
  = let isoDomain = Func dss (Lift1 [Lift1 dts])
        ...
        isoRouteInv (Rep1 lfi [(!(Rep1 mfi hfsi))]) = Rep2 lfi mfi hfsi
      in map isoRouteInv isoSuccs
 
spSuccsFrel :: [Domain] -> FrontierElem -> [FrontierElem]
spSuccsFrel [(!(!d))] (MkFrel [(!r)])
  = map (\ rs -> MkFrel [rs]) (spSuccsR d r)
spSuccsFrel ds (!(MkFrel rs))
  = map MkFrel
      (myListVariants (map avBottomR ds) (myZipWith2 spSuccsR ds rs))
 
spLEmb :: [Rep] -> Frontier -> Rep
spLEmb hfBottoms (!h) = Rep1 h hfBottoms
 
spLLift :: [Route] -> [Domain] -> [Domain] -> [Rep] -> Rep
spLLift initTops initDss hfDomains (!hfs_reps)
  = let lf_arity = length initTops
        (!zapped_hfs)
          = myZipWith2 (spLLift_aux lf_arity initTops initDss) hfDomains
              hfs_reps
        new_lf
          = case foldr1 avLUBrep zapped_hfs of
                RepTwo fr -> fr
      in Rep1 new_lf hfs_reps
...
spLLift_aux des_arity initTops initDss (Func (!dss) (Lift1 dts))
  (Rep1 (!lf) hfs)
  = spLLift_reduce_arity_as_top des_arity initTops initDss
      (RepTwo lf)
spLLift_aux (!(!des_arity)) initTops initDss (Func dss (Lift2 dts))
  ((!(Rep2 lf mf (!hfs))))
  = spLLift_reduce_arity_as_top des_arity initTops initDss
      (RepTwo lf)
 
spLLift_reduce_arity_as_top ::
                            Int -> [Route] -> [Domain] -> Rep -> Rep
spLLift_reduce_arity_as_top des_arity initTops initDss
  ...
  | ar > des_arity =
    let shorten (MkFrel rs) = MkFrel (take des_arity rs)
      ...
      in RepTwo (Min1Max0 des_arity new_f1 new_f0)
...
spPredsR ((Lift1 (![d]))) (Up1 (!([r])))
  = if avIsBottomR r then [Stop1] else
      map (\ rp -> Up1 [rp]) (spPredsR d r)
spPredsR ((!((!(Lift1 ds))))) (Up1 rs)
  = if myAll avIsBottomR rs then [Stop1] else
      map Up1
        (myListVariants (map avTopR ds) (myZipWith2 spPredsR ds rs))
spPredsR (!(Lift2 ds)) Stop2 = []
spPredsR ((!(Lift2 (!ds)))) Up2 = [Stop2]
spPredsR (Lift2 [d]) ((!(UpUp2 [r])))
  = if avIsBottomR r then [Up2] else
      map (\ rp -> UpUp2 [rp]) (spPredsR d r)
spPredsR (Lift2 ds) (((!(UpUp2 rs))))
  = if myAll avIsBottomR rs then [Up2] else
      map UpUp2
        (myListVariants (map avTopR ds) (myZipWith2 spPredsR ds rs))
spPredsR (d@(Func _ (!_))) (Rep r) = map Rep (spPredsRep d r)
...
spGEmb_aux lf (!Two) = RepTwo lf
spGEmb_aux lf (((!(Lift1 dss))))
  = Rep1 lf (map (spGEmb_aux lf) dss)
spGEmb_aux lf (Lift2 (!dss)) = Rep2 lf lf (map (spGEmb_aux lf) dss)
...
spGEmb_increase_arity_ignore f [] = RepTwo f
spGEmb_increase_arity_ignore (Min1Max0 ar f1 f0) dss
  = let tops = map avTopR dss
      ...
      in RepTwo (Min1Max0 (ar + length dss) new_f1 new_f0)
 
spMax0FromMin1 :: [Domain] -> [FrontierElem] -> [FrontierElem]
spMax0FromMin1 dss f1 = spMax0FromMin1_aux (map avTopR dss) dss f1
spMax0FromMin1_aux tops dss ((!f1))
  = sort
      (foldr avLUBmax0frontier [MkFrel tops] (map (spPredsFrel dss) f1))
...
spMin1FromMax0_aux bottoms (!dss) (!f0)
  = sort
      (foldr avGLBmin1frontier [MkFrel bottoms]
