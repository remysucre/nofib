...
deFreeVars (ECase (!e) alts)
  ...
  where ((!e')) = deFreeVars e
        ...
        f (!(t, ((!ns), e)))
          = utSetSubtraction (deFreeVarsOf e) (utSetFromList ns)
deFreeVars (!(ELam (args) (!body)))
  = (utSetSubtraction (deFreeVarsOf body') (utSetFromList args),
     ALam args body')
...
deDepthFirstSearch ::
                     (Ord a) => (a -> [a]) -> (Set a, [a]) -> [a] -> (Set a, [a])
deDepthFirstSearch = foldl . search
  where search relation (visited, (!(sequence))) vertex
...
deSpanningSearch ::
                   (Ord a) =>
                   (a -> [a]) -> (Set a, [Set a]) -> [a] -> (Set a, [Set a])
deSpanningSearch = foldl . search
  where search (!relation) (visited, utSetSequence) (!vertex)
          | utSetElementOf vertex visited = (visited, utSetSequence)
          | otherwise =
            (visited', utSetFromList (vertex : sequence) : utSetSequence)
          where (!(visited', (!sequence)))
                  = deDepthFirstSearch relation
...
deScc ins outs = spanning . depthFirst
  where depthFirst
          = second . deDepthFirstSearch outs (utSetEmpty, [])
...
deDepends (free, AAp (e1) (!e2))
  = EAp (deDepends e1) (deDepends e2)
deDepends (!((!free), ACase (!body) alts))
  = ECase (deDepends body)
      [(t, (ns, deDepends e)) | (t, ((!(ns, e)))) <- alts]
...
deDepends ((free, ALet isRec defns body))
  ...
  where binders = deBindersOf defns
        ...
        (!defnGroups)
          = [[(n, utSureLookup defns "depends`defnGroups" n) | n <- ns] |
             ns <- components]
deElet isRec dfs e
  = if not isRec || nonRec dfs then ELet False dfs' e else
      ELet True dfs' e
