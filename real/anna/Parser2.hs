...
paThen2 :: (a -> b -> c) -> Parser a -> Parser b -> Parser c
paThen2 combine p1 p2 toks
  = let p1parse = p1 toks
        (!p2parse) = p2 (paGetRest p1parse)
      in
      if paFailed p1parse then PFail (paGetRest p1parse) else
        if paFailed p2parse then PFail (paGetRest p2parse) else
          POk (combine (paGetItem p1parse) (paGetItem p2parse))
            (paGetRest p2parse)
...
paThen3 combine (!p1) p2 p3 toks
  = let (!p1parse) = p1 toks
      ...
      if paFailed p1parse then PFail (paGetRest p1parse) else
        if paFailed p2parse then PFail (paGetRest p2parse) else
          if paFailed p3parse then PFail (paGetRest p3parse) else
            POk
              (combine (paGetItem p1parse) (paGetItem p2parse)
                 (paGetItem p3parse))
              (paGetRest p3parse)
...
paThen4 combine p1 p2 p3 p4 toks
  = let p1parse = p1 toks
      ...
      if paFailed p1parse then PFail (paGetRest p1parse) else
        if paFailed p2parse then PFail (paGetRest p2parse) else
          if paFailed p3parse then PFail (paGetRest p3parse) else
            if paFailed p4parse then PFail (paGetRest p4parse) else
              POk
                (combine (paGetItem p1parse) (paGetItem p2parse)
                   (paGetItem p3parse)
                   (paGetItem p4parse))
...
paZeroOrMore (!p) toks
  = let pParse = p toks
      ...
      in
      if paFailed pParse then POk [] toks else
        if paFailed zmParse then POk [paGetItem pParse] pUnused else
          POk ((paGetItem pParse) : paGetItem zmParse) zmUnused
 
paOneOrMore :: Parser a -> Parser [a]
paOneOrMore p = paThen2 (:) p (paZeroOrMore p)
 
paOneOrMoreWithSep :: Parser a -> Parser b -> Parser [a]
paOneOrMoreWithSep p psep toks
  = let (!pParse) = p toks
      ...
      in
      if paFailed pParse then PFail toks else
        if paFailed sParse then POk [paGetItem pParse] pRest else
          if paFailed mParse then POk [paGetItem pParse] pRest else
            POk ((paGetItem pParse) : paGetItem mParse) mRest
 
paApply :: Parser a -> (a -> b) -> Parser b
paApply p f toks
  = let pParse = p toks in
      if paFailed pParse then PFail (paGetRest pParse) else
        POk (f (paGetItem pParse)) (paGetRest pParse)
 
paSat :: (String -> Bool) -> Parser String
paSat pred [] = PFail []
paSat pred ((!(n, t)) : (!toks))
  | pred t = POk t toks
...
paIsCname (!s)
  = ('A' <= (head s)) &&
      ((head s) <= 'Z') && not (s `elem` paKeywords)
...
paTDefExpr
  = paAlts
      [((== "("), paTDefExpr2), (paIsName, paApply paName TDefVar)]
  where (!paTDefExpr2)
          = paThen3 g (paLit "(") paTDefExpr3 (paLit ")")
        g a b c = b
        paTDefExpr3 = paThen2 h paName (paZeroOrMore paTDefExpr)
        h a b = TDefCons a b
paScdefs = paOneOrMore (paWithTrailingSemi paSc)
paSc
  = paThen4 mk_sc paName (paZeroOrMore paName) (paLit "=") paExpr
  where mk_sc sc args eq rhs = (sc, (args, rhs))
paExpr
  = paAlts
      [((== "let"), paLet), ((== "letrec"), paLetrec),
       ((== "case"), paCase), ((== "\\"), paLambda),
       ((const True), paExpr1)]
paLet = paThen4 mk_let (paLit "let") paDefns (paLit "in") paExpr
  where mk_let lett defns (!inn) expr = ELet False defns expr
paLetrec
  = paThen4 mk_letrec (paLit "letrec") paDefns (paLit "in") paExpr
  where mk_letrec letrecc defns inn (!(!expr)) = ELet True defns expr
paDefns = paOneOrMoreWithSep paDefn (paLit ";")
paDefn = paThen3 mk_defn paName (paLit "=") paExpr
  where mk_defn (!(!var)) equals rhs = (var, rhs)
paCase
  = paThen4 mk_case (paLit "case") paExpr (paLit "of") paAlters
  where mk_case kase e (!ov) alts = ECase e alts
paAlters
  = paThen2 const (paOneOrMoreWithSep paAlter (paLit ";"))
      (paLit "end")
paAlter
  = paThen4 mk_alt paCname (paZeroOrMore paName) (paLit "->") paExpr
  where mk_alt tag (!args) arrow rhs = (tag, (args, rhs))
paLambda
  = paThen4 mk_lam (paLit "\\") (paOneOrMore paName) (paLit "->")
      paExpr
  where mk_lam lam vars dot expr = ELam vars expr
...
paExpr2c
  = paAlts
      [((== "&"), paThen2 FoundOp (paLit "&") paExpr2),
       (const True, paEmpty NoOp)]
paExpr3 = paThen2 paAssembleOp paExpr4 paExpr3c
paExpr3c
  = paAlts
      [(paIsRelop, paThen2 FoundOp paRelop paExpr4),
       (const True, paEmpty NoOp)]
paExpr4 = paThen2 paAssembleOp paExpr5 paExpr4c
paExpr4c
  = paAlts
      [((== "+"), paThen2 FoundOp (paLit "+") paExpr4),
...
paExpr6 = (paOneOrMore paAtomic) `paApply` mk_ap_chain
  where mk_ap_chain ((((!(fn : args))))) = foldl EAp fn args
paAtomic
  = paAlts
      [(paIsCname, paConstr), ((== "("), paBracExpr),
       (paIsName, paName `paApply` EVar), (paIsNum, paNum `paApply` ENum)]
paBracExpr = paThen3 mk_brack (paLit "(") paExpr (paLit ")")
  where mk_brack (!open) expr (!close) = expr
paConstr = paApply paCname EConstr
paAssembleOp e1 NoOp = e1
paAssembleOp e1 (FoundOp (op) (!e2)) = EAp (EAp (EVar op) e1) e2
 
paProgramToAtomic :: CoreProgram -> AtomicProgram
paProgramToAtomic (tds, scdefs) = (tds, ce)
  where (!ce)
          = ELet True [(name, ELam ns b) | ((!name), (ns, b)) <- scdefs]
              (ENum 42)
 
paValidTypeDefs :: [TypeDef] -> TypeDependancy -> [Char]
      ...
      if not uniqueParNames then "Non-unique parameter names" else
        if not uniqueCNames then "Non-unique constructor names" else
          if not balanced then
            "Declared parameters do not match used parameters" else
            if not allDefined then "Undefined types are present" else
              if not rightArity then "Types are used at wrong arities" else
                if not allSimple then "Perverse type definitions are present" else
                  ""
  where arityMap = map f tds
          where f ((!((!tname))), tvs, cal) = (tname, length tvs)
        (!allTNames) = map f tds
          where f ((!tname), tvs, cal) = tname
        allCNames = concat (map f tds)
          where f (tname, tvs, cal) = map first cal
        uniqueTNames = length allTNames == ((length . nub) allTNames)
        (!uniqueParNames) = and (map f tds)
          where f (tname, tvs, (!cal)) = length tvs == ((length . nub) tvs)
        uniqueCNames = length allCNames == ((length . nub) allCNames)
        balanced = and (map isBalanced tds)
          where tvsIn (TDefVar n) = [n]
        ...
        allDefined
          = utSetSubsetOf (utSetFromList (concat (map mdFreeTVarsIn tds)))
              (utSetFromList allTNames)
        rightArity = and (map f tds)
          where f (tname, tvs, cal) = and (map (g . second) cal)
...
paParse fileContents
  = if typeDefErrors == "" then (dependResult, (typeDefs, mainExpr))
      else myFail typeDefErrors
  where (typeDefs, (!mainExpr)) = paProgramToAtomic parsedProgram
