...
amUpCloseOfMinf ((!d)) q@(x : _)
  = x :
      (amUpCloseOfMinf d (avMinR [y \/ z | y <- q, z <- spSuccsR d x]))
...
amEqualPoints (d1, (!(r1))) ((!(d2)), r2)
  = if d1 == d2 then r1 == r2 else
      panic "Comparing points in different domains."
 
amIsaHOF :: Domain -> Bool
amIsaHOF ((((!(Func dss dt)))))
  = amContainsFunctionSpace dt || myAny amContainsFunctionSpace dss
 
amContainsFunctionSpace :: Domain -> Bool
amContainsFunctionSpace Two = False
amContainsFunctionSpace (Lift1 dss)
  = myAny amContainsFunctionSpace dss
amContainsFunctionSpace (Lift2 dss)
  = myAny amContainsFunctionSpace dss
amContainsFunctionSpace ((!(Func _ (_)))) = True
 
amIsDataFn :: Domain -> Bool
amIsDataFn (!((!(Func _ ((!dt))))))
  = not (amContainsFunctionSpace dt)
...
amStrongNormalise Two = Two
amStrongNormalise ((!(!(Lift1 ds))))
  = Lift1 (map amStrongNormalise ds)
amStrongNormalise (!(Lift2 ds)) = Lift2 (map amStrongNormalise ds)
