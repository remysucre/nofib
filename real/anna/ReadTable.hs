...
rtReadTable (!s)
  = case rtTable (rtLex 1 s) of
        PFail [] -> myFail "Unexpected end of lattice table"
        PFail ((n, t) : _) -> myFail
                                ("Syntax error in lattice table, line " ++ show n ++ ".")
        POk tab [] -> tab
        POk tab ((n, t) : _) -> myFail
                                  ("Syntax error in lattice table, line " ++ show n ++ ".")
...
rtLex n ('F' : ((!'u') : (!('n' : (('c' : cs))))))
  = (n, "F") : rtLex n cs
rtLex n ('L' : (('i') : (!('f' : ('t' : ((!'1') : cs))))))
  = (n, "L") : rtLex n cs
rtLex (!n) (('L') : ((!'i') : (!('f' : (!('t' : (!('2' : cs))))))))
  = (n, "M") : rtLex n cs
rtLex (!(!n)) (c : cs)
  | isDigit c =
  ...
  | otherwise =
    myFail
      ("Illegal character " ++
         show c ++ " in lattice table, line " ++ show n ++ ".")
rtPWithComma p = paThen2 (\ a b -> a) p (paLit ",")
rtListMain p
  = paAlts
      [((== "]"), paApply (paLit "]") (const [])),
       (const True,
        paThen3 (\ a b c -> a ++ [b]) (paZeroOrMore (rtPWithComma p)) p
          (paLit "]"))]
...
rtDomain
  = paAlts
      [((== "("),
       ...
       ((== "F"),
        paThen3 (\ a b c -> Func b c) (paLit "F") rtListDomain rtDomain)]
rtPair (!pa) pb
  = paThen4 (\ (!a) b (!c) d -> (b, d)) (paLit "(") pa (paLit ",")
      (paThen2 (\ a b -> a) pb (paLit ")"))
rtTable = rtList (rtPair rtDomain paNum)