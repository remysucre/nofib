...
coMakeConstructorInstance ::
                          Bool -> [ConstrElem] -> DExpr -> DSubst -> Route
coMakeConstructorInstance (!mi) cargs simplest_init usage
  = let (doCAFkludge, simplest)
        ...
        actual = dxApplyDSubst usage simplest
        ...
        points_not_below_structure_point
          = case (recursive, actualTarget) of
                ...
                ((False, (!Two))) -> [One]
        tagTable
          = [(p, arg_bottoms) | (!(!p)) <- points_below_structure_point] ++
              [(p, [MkFrel (map (magic p) cargs)]) |
               p <- points_not_below_structure_point]
        (!arg_bottoms) = [MkFrel (map avBottomR actualSources)]
        magic p ConstrRec = p
        ...
        xpts p
          ...
          | otherwise =
            case p of
                Up1 rs -> rs
      in
      if doCAFkludge then apPapConst (coCGen_aux mi tagTable actual) else
        Rep (coCGen_aux mi tagTable actual)
 
coCGen_aux :: Bool -> AList Route [FrontierElem] -> Domain -> Rep
coCGen_aux mi (!tt) (Func dss Two)
  = let f1 = sort (utSureLookup tt "coCGen_aux(1)" One)
        f0 = spMax0FromMin1 dss f1
        ar
          = case head (f1 ++ f0) of
                MkFrel fels -> length fels
      in RepTwo (Min1Max0 ar f1 f0)
coCGen_aux mi tt (Func (dss) (Lift1 dts))
  = let (!lf_f1)
          = sort (utSureLookup tt "coCGen_aux(2)" (Up1 (map avBottomR dts)))
...
coCGen_aux (!mi) tt (Func dss (Lift2 (!dts)))
  = let lf_f1 = sort (utSureLookup tt "coCGen_aux(2)" Up2)
        ...
        (mf_f1)
          = sort
              (utSureLookup tt "coCGen_aux(3)" (UpUp2 (map avBottomR dts)))
        mf_f0 = spMax0FromMin1 dss mf_f1
        lf_ar = length dss
        newtt = [(rs, fels) | ((!(UpUp2 (rs))), fels) <- tt]
      in
      Rep2 (Min1Max0 lf_ar lf_f1 lf_f0) (Min1Max0 lf_ar mf_f1 mf_f0)
        (coCGen_aux_cross mi newtt dss dts)
coCGen_aux mi (!tt) (!(Func dss gDomain@(Func (!dss2) dt)))
  = let newtt = map makenewtt (amAllRoutes dt)
        makenewtt x
          = (x,
             avMinfrel
               [MkFrel (xs ++ ys) | (g, min_args_to_get_g) <- tt,
...
coCGen_aux_cross ::
                 Bool ->
                   AList [Route] [FrontierElem] -> [Domain] -> [Domain] -> [Rep]
coCGen_aux_cross mi tt dss (!dts)
  = let (!numberOfDimensions) = length dts
        doOneDimension n = coCGen_aux mi (fixtt n) (Func dss (dts ## n))
        fixtt n
          = let (!thisDimPoints) = taddall [] tt
                taddall acc [] = acc
                ...
                tadd ::
                     Route ->
                       [FrontierElem] ->
                         AList Route [[FrontierElem]] -> AList Route [[FrontierElem]]
                tadd r (!fel) (![]) = [(r, [fel])]
                tadd r (!fel) (this@((!rr), fels) : rest)
                  | r == rr = (rr, fel : fels) : rest
                  | otherwise = this : tadd r fel rest
                fixedtt
                  = map2nd (foldr avLUBmin1frontier [MkFrel (map avTopR dss)])
                      thisDimPoints
              in fixedtt
      in map doOneDimension (0 `myIntsFromTo` (numberOfDimensions - 1))