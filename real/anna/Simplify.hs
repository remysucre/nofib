...
siVectorise (HLam vs e) = HLam vs (siVectorise e)
...
siSimplify (!hexpr)
  = let hexpr_after_one_cycle = siHOpt hexpr in
      if hexpr == hexpr_after_one_cycle then hexpr else
        siSimplify hexpr_after_one_cycle
...
siHOpt_meet (!es)
  = let presimplified = map siHOpt es
        litsplit (!((lits, (!nonlits)))) ((!(!(HPoint p))))
          = (p : lits, nonlits)
        litsplit (lits, (!nonlits)) other = (lits, other : nonlits)
        ((lits, (!nonlits))) = foldl litsplit ([], []) presimplified
        onelit = foldr1 (\/) lits
      in
      if null lits then HMeet presimplified else
        if avIsTopR onelit then HPoint onelit else
          if avIsBottomR onelit then aeMkMeet (HPoint onelit) nonlits else
            aeMkMeet (HPoint onelit) ((HPoint onelit) : nonlits)
 
siHOpt_app :: HExpr Naam -> HExpr Naam -> HExpr Naam
siHOpt_app (HTable t) (((!(HPoint p))))
  = siHOpt (utSureLookup t "siHOpt_app" p)
siHOpt_app (HPoint p1) (HPoint p2) = HPoint (apApply p1 [p2])
