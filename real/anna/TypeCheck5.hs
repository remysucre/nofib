...
tcMapAnnExpr :: (a -> b) -> AnnExpr c a -> AnnExpr c b
tcMapAnnExpr (!f) (ann, node) = (f ann, mapAnnExpr' node)
  where mapAnnExpr' (AVar v) = AVar v
        ...
        mapAnnDefn (naam, expr) = (naam, tcMapAnnExpr f expr)
        mapAnnAlt (naam, (!(pars, resExpr)))
          = (naam, (pars, tcMapAnnExpr f resExpr))
 
tcSubstAnnTree :: Subst -> AnnExpr Naam TExpr -> AnnExpr Naam TExpr
tcSubstAnnTree phi (!tree) = tcMapAnnExpr (tcSub_type phi) tree
 
tcTreeToEnv :: AnnExpr Naam TExpr -> TypeEnv
tcTreeToEnv (!tree) = t2e tree
  where t2e (!(nodeType, node)) = t2e' node
        t2e' ((!(AVar ((!v))))) = []
        t2e' (!(ANum (!n))) = []
        t2e' (AConstr c) = []
        t2e' ((!(AAp ae1 (ae2)))) = (t2e ae1) ++ (t2e ae2)
        t2e' (ALam cs e) = t2e e
        t2e' (((ALet rf dl (!me)))) = (concat (map aFN dl)) ++ (t2e me)
        t2e' (!((((!(ACase sw alts))))))
          = (t2e sw) ++ (concat (map (t2e . second . second) alts))
        aFN ((!(naam)), ((!tijp), body)) = (naam, tijp) : (t2e' body)
 
tcShowtExpr :: TExpr -> [Char]
tcShowtExpr (!t) = pretty' False t
  where pretty' b (((!(TVar tvname))))
          = [' ', toEnum (96 + (lookup tvname tvdict))]
        pretty' b (TCons "int" []) = " int"
        pretty' (!b) (TCons "bool" []) = " bool"
        pretty' b (TCons "char" []) = " char"
        pretty' True ((!(TArr t1 (!t2))))
          = " (" ++ (pretty' True t1) ++ " -> " ++ (pretty' False t2) ++ ")"
        pretty' False (TArr (!t1) t2)
          = (pretty' True t1) ++ " -> " ++ (pretty' False t2)
        pretty' (!b) (!((TCons notArrow (!cl))))
          = " (" ++ notArrow ++ concat (map (pretty' True) cl) ++ ")"
        lookup tvname [] = panic "tcShowtExpr: Type name lookup failed"
        lookup (!tvname) (t : ts)
          | t == tvname = 1
...
tcPretty :: (Naam, TExpr) -> [Char]
tcPretty (naam, tipe)
  = "\n   " ++ (ljustify 25 (naam ++ " :: ")) ++ (tcShowtExpr tipe)
...
tcCheck baseTypes ns (tdefs, expr)
  = if good tcResult then (fullEnvWords, Ok (rootTree, fullEnv)) else
      ("", Fail "No type")
  where (!tcResult)
          = tc (tdefs ++ builtInTypes) (baseTypes ++ finalConstrTypes)
              finalNs
              expr
        good ((!(Ok (x)))) = True
        good (!(Fail x2)) = False
        (rootSubst, rootType, annoTree) = f tcResult
          where f ((!(Ok x))) = x
        rootTree = tcSubstAnnTree rootSubst annoTree
        (!rootEnv) = tcTreeToEnv rootTree
        (!fullEnv) = rootEnv ++ map f finalConstrTypes
          where f (naam, (Scheme vs t)) = (naam, t)
...
tcTvars_in :: TExpr -> [TVName]
tcTvars_in t = tvars_in' t []
  where tvars_in' ((!(TVar (!x)))) l = x : l
        tvars_in' (TCons y ts) l = foldr tvars_in' l ts
        tvars_in' (TArr t1 t2) (!l) = tvars_in' t1 (tvars_in' t2 l)
 
tcApply_sub :: Subst -> TVName -> TExpr
tcApply_sub phi tvn
  = if TVar tvn == lookUpResult then TVar tvn else
      tcSub_type phi lookUpResult
  where lookUpResult = utLookupDef phi tvn (TVar tvn)
 
tcSub_type :: Subst -> TExpr -> TExpr
tcSub_type (!phi) ((!(!(!((!(TVar tvn))))))) = tcApply_sub phi tvn
tcSub_type phi ((!(TCons tcn (ts))))
  = TCons tcn (map (tcSub_type phi) ts)
tcSub_type phi (!(TArr t1 ((!t2))))
  = TArr (tcSub_type phi t1) (tcSub_type phi t2)
 
tcScomp :: Subst -> Subst -> Subst
tcScomp sub2 sub1 = sub1 ++ sub2
 
tcId_subst :: Subst
tcId_subst = []
 
tcDelta :: TVName -> TExpr -> Subst
...
tcExtend :: Subst -> TVName -> TExpr -> Reply Subst Message
tcExtend phi (!tvn) t
  | t == TVar tvn = Ok phi
  | tvn `notElem` (tcTvars_in t) = Ok ((tcDelta tvn t) `tcScomp` phi)
  | otherwise =
    myFail
...
tcUnify :: Subst -> (TExpr, TExpr) -> Reply Subst Message
tcUnify phi ((!(TVar ((!tvn)))), t)
  = if phitvn == TVar tvn then tcExtend phi tvn phit else
      tcUnify phi (phitvn, phit)
  where phitvn = tcApply_sub phi tvn
        phit = tcSub_type phi t
tcUnify phi (p@((!(TCons _ _))), q@(TVar _)) = tcUnify phi (q, p)
...
tcUnify (!phi) (TCons tcn ts, (!(TCons tcn' ts')))
  | tcn == tcn' = tcUnifyl phi (ts `zip` ts')
tcUnify phi ((!t1), (!(t2)))
  = myFail
      ("Type error in source program:\n\n" ++
         "Cannot unify\n      " ++
...
tcUnifyl phi (!eqns) = foldr unify' (Ok phi) eqns
  where unify' eqn (Ok phi) = tcUnify phi eqn
        unify' eqn (!(Fail (m))) = Fail m
 
tcMergeSubs :: Subst -> Subst
tcMergeSubs phi
  = if newBinds == [] then unifiedOlds else
      tcMergeSubs (unifiedOlds ++ newBinds)
  where (!(newBinds, unifiedOlds)) = tcMergeSubsMain phi
 
tcMergeSubsMain :: Subst -> (Subst, Subst)
tcMergeSubsMain phi
  = (concat newUnifiersChecked,
     zip oldVars (tcOldUnified newUnifiersChecked oldGroups))
  where oldVars = nub (utDomain phi)
...
tcCheckUnifier (Fail (!m)) = panic ("tcCheckUnifier: " ++ m)
...
tcUnknowns_scheme :: TypeScheme -> [TVName]
tcUnknowns_scheme (Scheme scvs t) = tcTvars_in t `tcBar` scvs
 
tcBar :: (Eq a) => [a] -> [a] -> [a]
tcBar xs ys = [x | x <- xs, not (x `elem` ys)]
 
tcSub_scheme :: Subst -> TypeScheme -> TypeScheme
tcSub_scheme phi (!((!(Scheme scvs (t)))))
  = Scheme scvs (tcSub_type (tcExclude phi scvs) t)
  where tcExclude phi scvs
          = [(n, e) | ((n, (!e))) <- phi, not (n `elem` scvs)]
 
tcCharVal :: AList Naam b -> Naam -> b
tcCharVal (!al) k
  = utLookupDef al k (panic ("tcCharVal: no such variable: " ++ k))
 
tcUnknowns_te :: TcTypeEnv -> [TVName]
tcUnknowns_te gamma
  = concat (map tcUnknowns_scheme (utRange gamma))
 
tcSub_te :: Subst -> TcTypeEnv -> TcTypeEnv
tcSub_te phi gamma
  = [(x, tcSub_scheme phi st) | (!(x, (!st))) <- gamma]
 
tcNext_name :: TypeNameSupply -> TVName
tcNext_name ns@((f), (!s)) = ns
 
tcDeplete :: TypeNameSupply -> TypeNameSupply
tcDeplete (f, s) = (f, tcNSSucc s)
 
tcSplit :: TypeNameSupply -> (TypeNameSupply, TypeNameSupply)
tcSplit (f, s) = ((f2, [0]), (tcNSSucc f2, [0]))
  where f2 = tcNSDouble f
 
tcName_sequence :: TypeNameSupply -> [TVName]
tcName_sequence ns
  = tcNext_name ns : tcName_sequence (tcDeplete ns)
...
tcNSDouble :: [Int] -> [Int]
tcNSDouble [] = []
tcNSDouble (n : ns) = 2 * n' : ns'
  where n'
          | n > tcNSdlimit = n - tcNSdlimit
          | otherwise = n
        (!ns')
          | n' == n = tcNSDouble ns
...
tc ::
   [TypeDef] ->
     TcTypeEnv -> TypeNameSupply -> CExpr -> Reply TypeInfo Message
tc (!tds) (!gamma) ns (((!(((!(ENum (!n))))))))
  = Ok (tcId_subst, TCons "int" [], (TCons "int" [], ANum n))
tc tds gamma (!ns) (EVar x) = tcvar tds gamma ns x
tc tds gamma (!ns) (!((!(EConstr c)))) = tcvar tds gamma ns c
tc tds (!gamma) ns (EAp e1 e2) = tcap tds gamma ns e1 e2
tc (!tds) gamma ns (ELam [] e) = tc tds gamma ns e
tc (!tds) gamma ns (ELam (!([(!x)])) e) = tclambda tds gamma ns x e
tc (!tds) gamma ns (!(ELam ((!x) : ((!(((!y)) : xs)))) (!e)))
  = tclambda tds gamma ns x (ELam (y : xs) e)
tc tds gamma ns (ELet recursive dl e)
  = if not recursive then tclet tds gamma ns xs es e else
      tcletrec tds gamma ns xs es e
  where (xs, es) = unzip2 dl
tc tds gamma ns (ECase (switch) alts)
  = tccase tds gamma ns switch constructors arglists exprs
  where ((!constructors), alters) = unzip2 alts
        (arglists, exprs) = unzip2 alters
 
tcConstrTypeSchemes ::
                    TypeNameSupply ->
                      TypeDef -> (TypeNameSupply, AList Naam TypeScheme)
tcConstrTypeSchemes ns (tn, stvs, cal)
  = (finalNameSupply, map2nd enScheme cAltsCurried)
  where newTVs = tcNewTypeVars (tn, stvs, cal) ns
...
tccase ::
       [TypeDef] ->
         TcTypeEnv ->
           TypeNameSupply ->
             CExpr -> [Naam] -> [[Naam]] -> [CExpr] -> Reply TypeInfo Message
tccase (!tds) gamma ns sw (!cs) (!(!als)) res
  = if length tdCNames /= length (nub cs) then
      myFail "Error in source program: missing alternatives in CASE" else
      tccase1 tds gamma ns1 sw reOals reOres newTVs tdInUse
  where tdInUse = tcGetTypeDef tds cs
        ...
        [TypeDef] ->
          TcTypeEnv ->
            TypeNameSupply ->
              CExpr ->
                [[Naam]] ->
                  [CExpr] -> AList Naam TVName -> TypeDef -> Reply TypeInfo Message
tccase1 tds gamma (!ns) sw reOals reOres newTVs (!tdInUse)
  = tccase2 tds gamma ns2 sw reOals newTVs tdInUse rhsTcs
  where rhsGammas = tcGetAllGammas newTVs (tcK33 tdInUse) reOals
          ...
          TcTypeEnv ->
            TypeNameSupply ->
              CExpr ->
                [[Naam]] ->
                  AList Naam TVName ->
                    TypeDef -> [Reply TypeInfo Message] -> Reply TypeInfo Message
tccase2 tds gamma ns (!sw) reOals newTVs tdInUse (!rhsTcs)
  = tccase3 tds gamma ns sw reOals newTVs tdInUse rhsTcs phi_1_to_n
      tau_1_to_n
      phi_rhs
  where phi_1_to_n = map tcOk13sel rhsTcs
                  ...
                  AList Naam TVName ->
                    TypeDef ->
                      [Reply TypeInfo Message] ->
                        [Subst] -> [TExpr] -> Subst -> Reply TypeInfo Message
tccase3 (!(!tds)) gamma (!ns) sw reOals newTVs tdInUse (!rhsTcs)
  phi_1_to_n tau_1_to_n phi_rhs
  = Ok
      (phi_Big, tau_final,
       (tau_final,
        ACase tree_s (zip tdCNames (zip reOals annotatedRHSs))))
  where phi_sTau_sTree_s = tc tds gamma ns sw
        ...
        phi_lhs = tcUnify tcId_subst (tau_lhs, tau_s)
...
tcUnifySet :: Subst -> [TExpr] -> Reply Subst Message
tcUnifySet sub (e1 : []) = Ok sub
tcUnifySet sub (e1 : (e2 : [])) = tcUnify sub (e1, e2)
tcUnifySet sub ((!(e1 : (e2 : (e3 : es)))))
  = tcUnifySet newSub (e2 : e3 : es)
  where newSub = tcDeOksel (tcUnify sub (e1, e2))
 
tcNewTypeVars :: TypeDef -> TypeNameSupply -> AList Naam TVName
tcNewTypeVars (t, vl, c) ns = zip vl (tcName_sequence ns)
 
tcGetGammaN ::
            AList Naam TVName -> ConstrAlt -> [Naam] -> AList Naam TypeScheme
tcGetGammaN tvl ((cname, (!cal))) cparams
  = zip cparams (map (Scheme [] . tcTDefSubst tvl) cal)
 
tcTDefSubst :: AList Naam TVName -> TDefExpr -> TExpr
tcTDefSubst nameMap (TDefVar n) = f result
  where f (((!(Just tvn)))) = TVar tvn
        f Nothing = TCons n []
...
tcGetAllGammas ::
               AList Naam TVName ->
                 [ConstrAlt] -> [[Naam]] -> [AList Naam TypeScheme]
tcGetAllGammas tvl [] (![]) = []
tcGetAllGammas (!tvl) (calt : calts) (cparams : cparamss)
  = tcGetGammaN tvl calt cparams : tcGetAllGammas tvl calts cparamss
 
tcGetTypeDef :: [TypeDef] -> [Naam] -> TypeDef
tcGetTypeDef (!tds) cs
  = if length tdefset == 0 then
      myFail "Undeclared constructors in use" else
      if length tdefset > 1 then
        myFail "CASE expression contains mixed constructors" else
        head tdefset
  where (!tdefset)
          = nub
              [(tname, ftvs, cl) | (tname, ftvs, cl) <- tds, usedc <- cs,
...
tcl ::
    [TypeDef] ->
      TcTypeEnv ->
        TypeNameSupply ->
          [CExpr] -> Reply (Subst, [TExpr], [AnnExpr Naam TExpr]) Message
tcl tds gamma (!ns) [] = Ok (tcId_subst, [], [])
tcl tds gamma ns (e : es)
  = tcl1 tds gamma ns0 es (tc tds gamma ns1 e)
  where (ns0, ns1) = tcSplit ns
...
tcl2 (!phi) (t) ((Ok ((!psi), (!ts), annotatedEs))) (!annotatedE)
  = Ok
      (psi `tcScomp` phi, (tcSub_type psi t) : ts,
       annotatedE : annotatedEs)
 
tcvar ::
      [TypeDef] ->
        TcTypeEnv -> TypeNameSupply -> Naam -> Reply TypeInfo Message
tcvar tds (!gamma) ns x
  = Ok (tcId_subst, finalType, (finalType, AVar x))
  where scheme = tcCharVal gamma x
        finalType = tcNewinstance ns scheme
 
tcNewinstance :: TypeNameSupply -> TypeScheme -> TExpr
tcNewinstance ns ((Scheme scvs (!t))) = tcSub_type phi t
  where al = scvs `zip` (tcName_sequence ns)
        phi = tcAl_to_subst al
 
tcAl_to_subst :: AList TVName TVName -> Subst
tcAl_to_subst (!al) = map2nd TVar al
 
tcap ::
     [TypeDef] ->
       TcTypeEnv ->
         TypeNameSupply -> CExpr -> CExpr -> Reply TypeInfo Message
tcap tds (!gamma) (!ns) e1 e2
  = tcap1 tvn (tcl tds gamma ns' [e1, e2])
  where tvn = tcNext_name ns
        ns' = tcDeplete ns
tcap1 tvn (Fail m) = Fail m
tcap1 (!tvn) (Ok (phi, [t1, t2], [ae1, ae2]))
  = tcap2 tvn (tcUnify phi (t1, t2 `TArr` (TVar tvn))) [ae1, ae2]
tcap2 tvn (((!(Fail m)))) (![ae1, ae2]) = Fail m
tcap2 tvn ((!(Ok phi))) ((![ae1, (!ae2)]))
  = Ok (phi, finalType, (finalType, AAp ae1 ae2))
  where (!finalType) = tcApply_sub phi tvn
 
tclambda ::
         [TypeDef] ->
           TcTypeEnv ->
             TypeNameSupply -> Naam -> CExpr -> Reply TypeInfo Message
tclambda (!tds) gamma ns x (!e)
  = tclambda1 tvn x (tc tds gamma' ns' e)
  where ns' = tcDeplete ns
...
tclambda1 tvn (!x) (Ok (phi, t, annotatedE))
  ...
  where finalType = (tcApply_sub phi tvn) `TArr` t
...
tclet ::
      [TypeDef] ->
        TcTypeEnv ->
          TypeNameSupply ->
            [Naam] -> [CExpr] -> CExpr -> Reply TypeInfo Message
...
tclet1 tds gamma ns xs e ((!(Ok (phi, ts, rhsAnnExprs))))
  ...
  where gamma'' = tcAdd_decls gamma' ns0 xs ts
        gamma' = tcSub_te phi gamma
        (ns0, ns1) = tcSplit ns
tclet2 phi xs recFlag (Fail m) rhsAnnExprs = Fail m
tclet2 phi xs recFlag (Ok (!((phi', t, annotatedE))))
  (!rhsAnnExprs)
  = Ok
      (phi' `tcScomp` phi, t,
       (t, ALet recFlag (zip xs rhsAnnExprs) annotatedE))
 
tcAdd_decls ::
            TcTypeEnv -> TypeNameSupply -> [Naam] -> [TExpr] -> TcTypeEnv
tcAdd_decls gamma ns xs ts = (xs `zip` schemes) ++ gamma
  where (schemes) = map (tcGenbar unknowns ns) ts
...
tcletrec tds gamma ns xs es (!e)
  ...
  where (ns0, ns') = tcSplit ns
        ((!(!ns1)), ns2) = tcSplit ns'
        nbvs = tcNew_bvars xs ns2
tcNew_bvars xs ns = map tcNew_bvar (xs `zip` (tcName_sequence ns))
tcletrec1 (!((!tds))) gamma ns xs ((!nbvs)) e ((!(Fail m)))
  = (Fail m)
tcletrec1 tds gamma ns xs nbvs e ((Ok (phi, (!ts), rhsAnnExprs)))
  = tcletrec2 tds gamma' ns xs nbvs' e (tcUnifyl phi (ts `zip` ts'))
      rhsAnnExprs
  where ts' = map tcOld_bvar nbvs'
