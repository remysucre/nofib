  ...
  = let seed1_ok = 1 <= s1 && s1 <= 2147483562
        ...
        rands s1 s2
          = let k = s1 `div` 53668
                ...
                z = s1'' - s2''
              in
              if z < 1 then z + 2147483562 : rands s1'' s2'' else
                z : rands s1'' s2''
      in
      if seed1_ok && seed2_ok then rands s1 s2 else
        panic "utRandomInts: bad seeds"
 
utSCdexprs :: StaticComponent -> DExprEnv
utSCdexprs
  ((!(dexprs, domains, ((!constrelems)), freevars, (!flags), (lims),
      sizes)))
  = dexprs
 
utSCdomains :: StaticComponent -> DSubst
utSCdomains
  (dexprs, domains, constrelems, freevars, (!(flags)), lims, sizes)
  = domains
...
utLookupAll [] k' = []
utLookupAll (((!((!(k, v)) : bs)))) k'
  | k == k' = v : utLookupAll bs k'
...
utiLayn iss = utiLaynN 1 iss
  where  
        ...
        utiLaynN n ((!(is : (!isz))))
          = utiConcat
              [(utiLjustify 4 (utiAppend (utiNum n) (utiStr ") "))),
               (utiIndent is), (utiLaynN (n + 1) isz)]
...
utiFWNum width n = utiStr (utpspaces spaces_reqd digits)
  where digits = show n
        spaces_reqd
          | length digits >= width = 0
...
utiChar c (!rest) (indent) (!col)
  ...
  | otherwise =
    utpspaces (indent - col) (c : rest indent (indent + 1))
utiIndent (!iseq) oseq (!indent) col
  = iseq oseq' (max col indent) col
  where oseq' indent' col' = oseq indent col'
...
splitList p [] = ([], [])
splitList (!p) (x : (xs))
  = case splitList p xs of
        (ayes, noes) -> if p x then (x : ayes, noes) else (ayes, x : noes)
first (a, b) = a
second (a, b) = b
...
interleave e [] = []
interleave e [xs] = xs
interleave (!e) (!(xs : ((!(xs2 : xss)))))
  = xs ++ e ++ (interleave e (xs2 : xss))
...
doStatefulOp1 f initState initValue1 = f initValue1 initState
