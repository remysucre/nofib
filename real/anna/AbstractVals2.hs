...
avTopR_aux (!(Func dss Two)) = RepTwo (avTopR_aux_2 dss)
...
avBottomR_aux (Func dss Two) = RepTwo (avBottomR_aux_2 dss)
...
avIsBottomRep (!(RepTwo (Min1Max0 ar (!f1) f0))) = null f1
...
avIsTopRep :: Rep -> Bool
avIsTopRep (RepTwo (Min1Max0 ar f1 f0)) = null f0
avIsTopRep ((((!(Rep1 (!lf) hfs))))) = myAll avIsTopRep hfs
avIsTopRep (Rep2 lf (!mf) hfs) = myAll avIsTopRep hfs
...
avLEQR_list [] [] = True
avLEQR_list ((a1 : (![]))) ((!((!b1) : []))) = a1 << b1
avLEQR_list ((!((!a1) : ((a2) : [])))) (((b1 : (b2 : (![])))))
  = if a1 << b1 then a2 << b2 else False
avLEQR_list (!(((!((!(a1)) : ((!a2) : ((!(a3 : [])))))))))
  ((!(b1 : (b2 : ((!b3) : [])))))
  = if a1 << b1 then if a2 << b2 then a3 << b3 else False else False
avLEQR_list (a1 : (a2 : (a3 : (a4 : []))))
  ((!(b1 : (b2 : (((!(b3 : (b4 : [])))))))))
  = if a1 << b1 then
      if a2 << b2 then if a3 << b3 then a4 << b4 else False else False
      else False
avLEQR_list ((!a1) : ((a2) : (a3 : (a4 : as@(!(_ : _))))))
  ((b1 : ((!b2) : (!((b3 : ((!(b4 : bs@(_ : (!_)))))))))))
  = if a1 << b1 then
      if a2 << b2 then
        if a3 << b3 then if a4 << b4 then avLEQR_list as bs else False else
          False
        else False
      else False
...
avBelowEQfrontier (Min1Max0 (!ar1) f1a (!f0a))
  (Min1Max0 ar2 f1b f0b)
  = let outer [] = True
        outer (x : xs) = if inner x f1b then outer xs else False
        inner y [] = False
        inner y (z : (!zs))
          = if z `avBelowEQfrel` y then True else inner y zs
      in outer f1a
...
avBelowEQrep (!((!(Rep1 lf1 hfs1)))) ((((!(Rep1 lf2 hfs2)))))
  = avBelowEQfrontier lf1 lf2 && myAndWith2 avBelowEQrep hfs1 hfs2
avBelowEQrep (Rep2 lf1 mf1 hfs1) (Rep2 (!(lf2)) (!mf2) hfs2)
  = avBelowEQfrontier lf1 lf2 &&
      avBelowEQfrontier mf1 mf2 && myAndWith2 avBelowEQrep hfs1 hfs2
...
p@Up2 \/ Stop2 = p
p@(!Up2) \/ q = q
p@(!(UpUp2 (!rs1))) \/ (((!(UpUp2 rs2))))
  = UpUp2 (myZipWith2 (\/) rs1 rs2)
p@(!(UpUp2 (!rs1))) \/ q = p
...
avLUBfrontier (Min1Max0 ar1 f1a (!f0a))
  (((Min1Max0 ar2 f1b (!f0b))))
  = Min1Max0 ar1 (avLUBmin1frontier f1a f1b)
...
avLUBmin1frontier f1a (!f1b) = sort (foldr avMinAddPtfrel f1a f1b)
...
p@Up2 /\ q@Stop2 = q
p@Up2 /\ q = p
p@(!((!((!(UpUp2 (rs1))))))) /\ q@((!(UpUp2 rs2)))
  = UpUp2 (myZipWith2 (/\) rs1 rs2)
p@(UpUp2 rs1) /\ q = q
((!p@((!(Rep rep1))))) /\ q@((!(Rep rep2)))
  = Rep (avGLBrep rep1 rep2)
...
avGLBfrontier (Min1Max0 ar1 f1a f0a)
  ((!(((!(Min1Max0 ar2 f1b ((!f0b))))))))
  = Min1Max0 ar1 (avGLBmin1frontier f1a f1b)
...
avGLBmax0frontier ::
                  [FrontierElem] -> [FrontierElem] -> [FrontierElem]
avGLBmax0frontier f0a f0b = sort (foldr avMaxAddPtfrel f0a f0b)
 
avGLBmin1frontier ::
                  [FrontierElem] -> [FrontierElem] -> [FrontierElem]
avGLBmin1frontier f1a f1b
  = sort (avMinfrel [x `avLUBfrel` y | x <- f1a, y <- f1b])
avBelowMax0frel pt (f) = myAny (pt `avBelowEQfrel`) f
avAboveMin1frel pt f = myAny (`avBelowEQfrel` pt) f
avMinAddPtfrel (!x) ys
...
avBelowMax0R (!pt) f = myAny (pt <<) f
avAboveMin1R pt f = myAny (<< pt) f
avMinAddPtR x ys
...
avBelowMax0rep pt f = myAny (pt `avBelowEQrep`) f
avAboveMin1rep pt (!f) = myAny (`avBelowEQrep` pt) f
avMinAddPtrep x ys
