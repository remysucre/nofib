...
runalltests (!astart) (!astep) alim bstart bstep (!blim)
  = do runbench (+) (+) "(+)" astart astep alim astart astep alim
         ...
         (Integer -> Integer -> a) ->
           (Int -> Int -> b) ->
             String ->
               Integer ->
                 Integer -> Integer -> Integer -> Integer -> Integer -> IO ()
runbench jop (!iop) (!opstr) (!astart) (!astep) (!alim) (!bstart)
  bstep (!blim)
  = do intbench iop astart astep alim astart astep alim
...
integerbench op astart astep (!alim) (!bstart) (!bstep) blim
  = do seqlist
         ([a `op` b | a <- [astart, astart + astep .. alim],
...
intbench (!op) (!astart) (!astep) (!alim) bstart bstep blim
  = do seqlist
         ([a `op` b |
           a <- [fromInteger astart, fromInteger astart + fromInteger astep ..
                 fromInteger alim],
           b <- [fromInteger bstart, fromInteger astart + fromInteger bstep ..
                 fromInteger blim]])
       return ()
seqlist (![]) = return ()
