...
flop k xs = flopp xs (xs `advancePtr` k)
  where flopp (!i) j
          = when (i < j) $ swap i j >> flopp (incPtr i) (decPtr j)
        swap i j
          = do a <- peek i
               b <- peek j
               poke j a
...
genPermutations ::
                Int -> Int -> Int -> Ptr Word8 -> Ptr Word8 -> IO F
genPermutations (!n) !(!l) (!r) (!perm) (!count)
  = allocaArray n $
      \ destF ->
        do let upd j (!f) run
                 = do p0 <- peekElemOff perm 0
                      if p0 == 0 then increment perm count >> run f else
                        do copyArray destF perm n
                           increment perm count
                           flopS destF $
                             \ flops -> run (f `mappend` F (checksum j flops) flops)
      ...
      \ p ->
        allocaArray n $
          \ count ->
            allocaArray n $
              \ pp ->
                do mapM_ (\ i -> pokeElemOff p i (fromIntegral i)) [0 .. n - 1]
                   let go i (!idx)
                         = when (i >= 0) $
                             do let fi = facts !! i
                                ...
                                let go' j
                                      = when (j <= i) $
                                          do let jq = j + q
                                             pokeElemOff p j =<<
                                               peekElemOff pp (if jq <= i then jq else jq - i - 1)
                                             go' (j + 1)
                                go' 0
                                go (i - 1) r
                   go (n - 1) idx
...
main
  = do n <- fmap (read . head) getArgs
       ...
       let bk = fact `quot` 4
       vars <- forM [0, bk .. fact - 1] $
                 \ ix ->
                   do var <- newEmptyMVar
                      forkIO
                        (unrank ix n $
                           \ p -> genPermutations n ix (min fact (ix + bk)) p >=> putMVar var)
                      return var
